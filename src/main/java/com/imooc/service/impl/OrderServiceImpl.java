...
//            cartDTOList.add(cartDTO);
    ...
    public OrderDTO cancel(OrderDTO orderDTO) {
        ...
        if(CollectionUtils.isEmpty(orderDTO.getOrderDetailList())){
            log.error("【取消订单】 订单中暂无商品详情,orderDTO={}",orderDTO.toString());
    ...
    public OrderDTO paid(OrderDTO orderDTO) {
        ...
        if(!orderDTO.getPayStatus().equals(PayStatusEnum.WAIT.getCode())){
            log.error("【订单支付完成】 订单支付状态不正确, orderDTO={}",orderDTO.toString());
        ...
        OrderMaster updateResult= orderMasterRespository.save(orderMaster);
        if(updateResult ==null){
            log.error("【订单支付完成】 更新失败, orderMaster={}",orderMaster.toString());
            throw new SellException(ResultEnum.ORDER_UPDATE_FAIL);
        }
        return orderDTO;
    }
